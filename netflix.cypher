// Ejercitacion basada en el curso oficial de Neo4j

// Creacion del modelo e ingestacion de la data
...
// Consultas para comprobar el funcionamiento del modelo


// peliculas donde actuo tom Hanks
MATCH (p:Person {name: 'Tom Hanks'})-[:ACTED_IN]->(m)
RETURN m.title

// en cuantas peliculas actuo tom hanks 
MATCH (p:Person {name: 'Tom Hanks'})-[:ACTED_IN]->(m)
RETURN count(m.title)

// peliculas donde tom hanks actuo antes del 2000
MATCH (p:Person {name: 'Tom Hanks'})-[:ACTED_IN]->(m)
where m.released < 2000
RETURN m.title as titulo, m. released as anio

// seleccionar a los actores de The matrix y nacieron despues del 1960
MATCH (a:Person)-[:ACTED_IN]->(m:Movie)
WHERE m.title = 'The Matrix'  AND a.born > 1960
RETURN a.name, a.born

// peliculas donde actuo tom hanks en 2013
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
WHERE p.name = 'Tom Hanks'
AND m.year = 2013
RETURN m.title

// peliculas donde actuo tom hanks y cuya pelicula fue lanzada entre 2005 y 2010 
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
WHERE p.name = 'Tom Hanks'
AND  2005 <= m.released <= 2010
RETURN m.title, m.released


// devolver peliculas donde el actor sea tom hanks o el titulo de la pelicula sea capitan phillips
MATCH (p:Person)-[:ACTED_IN]->(m:Movie)
WHERE p.name = 'Tom Hanks'
OR m.title = 'Captain Phillips'
RETURN p.name, m.title

// devolver personas nacidas en 1980 y ordenar por fecha
MATCH (p:Person)
WHERE p.born = 1980
...
// devolver personas que nacieron antes de 1980, ordenado
MATCH (p:Person)
WHERE p.born < 1980
RETURN p.name AS name,
p.born AS birthDate
...
// devolver las 10 peliculas mas viejas 
MATCH (m:Movie)
RETURN m.title AS tittle,
m.released AS year
ORDER BY m.released
limit 10


// encontrar a las las personas que actuaron en una pelicula despues del 2000
MATCH (a:Person)-[:ACTED_IN]->(m:Movie)<-[:DIRECTED]-(d:Person)
WHERE m.year > 2000
RETURN a.name, m.title, d.name


// personas que actuaron y dirigieron un pelicula y fue lanzada despues del 2000
MATCH (a:Person)-[:ACTED_IN]->(m:Movie)
WHERE m.released > 2000
...
// peliculas que dirigio o actual Ton Hanks 
MATCH (p:Person)-[:DIRECTED | ACTED_IN]->(m:Movie)
WHERE p.name = 'Tom Hanks'
RETURN DISTINCT m.title, m.released
ORDER BY m.title

//  retornar las personas que se llaman Thomas
MATCH (p:Person)
WHERE p.name CONTAINS "Thomas"
RETURN p AS person ORDER BY p.name


// Contar las perliculas donde actuo Tom Hanks
MATCH (a:Person)-[:ACTED_IN]->(m:Movie)
